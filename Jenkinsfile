...
stage('Stop Existing Container'){
   powershell "docker ps -a -q --filter name=hello_world_app | FindStr -q . if($?) { docker stop ${container} }"
    }
    
stage('Remove Existing Container'){
   powershell "docker ps -a -q --filter name=hello_world_app | FindStr -q . if($?) { docker rm ${container} }"
    }
